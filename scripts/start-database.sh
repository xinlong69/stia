...
DB_USER=$(echo "$POSTGRES_URL" | awk -F'//' '{print $2}' | awk -F':' '{print $1}')
...
# determine which netcat command to use (nc or ncat)
...
# perform the port availability check if a tool was found
if [ -n "$NC_CMD" ]; then
  if $NC_CMD -z localhost "$DB_PORT" 2>/dev/null; then
    echo "Port $DB_PORT is already in use."
    exit 1
  fi
else
  echo "Warning: Unable to check if port $DB_PORT is already in use (netcat/ncat not installed)"
